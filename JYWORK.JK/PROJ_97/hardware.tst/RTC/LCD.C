...
/* Error Point : 0x00 ==> LCD Write End */

void L_printf(Byte x, Byte y, Byte *str, Byte Dly)
{
    lcd_disp(x,y);
...
void write_char(Byte s)   /* One charicter write to LCD */
{
    output(LCD_DATA,s);
    delay(100);
}

void L_shift(Byte move,Byte rot,Byte dly)
{
    Byte count;
...
void L_clrLine(Byte Line)
{
    Byte ii;
    
    for(ii=Line;ii<=COLUM;ii++)     L_printf(ii,Line," ",0);
}
