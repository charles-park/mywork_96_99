...
void vputche(int x,int y,char *buff,char color)
{
        int xwidth=xbyte;
...
void vputchh(int x,int y,char *buff,char color)
{
        int xwidth=xbyte;
...
hloop:  asm push cx
	...
	asm mov al,8
	asm out dx,ax
	asm mov ah,es:[di+1]		/* output middle part */
	asm mov es:[di+1],ah
	asm mov ax,bx
	asm mov al,ch
	asm not ah
	...
	asm mov al,8
	asm out dx,ax
	asm mov ah,es:[di+2]		/* output right part */
	asm mov es:[di+2],ah
	asm mov ax,bx
