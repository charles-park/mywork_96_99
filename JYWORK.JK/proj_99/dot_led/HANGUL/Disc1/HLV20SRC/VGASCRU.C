...
void vgascrup(int x1,int y1,int x2,int y2,int lines)
{
        int      xw=xbyte;
...
void vgasideup(int x,int y1,int y2,int lines,unsigned char mask)
{
        int      xw=xbyte;
        unsigned y_width=xw*lines;
...
void hgVScrUp(int x1,int y1,int x2,int y2,int lines,int bkcolor)
{
	int xs=x1/8+1;
	int xe=x2/8-1;
        ...
        vgascrup(xs,y1,xe,y2,lines);    /* mid scroll */

	vgasideup(xs-1,y1,y2,lines,lmask);
        vgasideup(xe+1,y1,y2,lines,rmask);

        hgVBoxFill(x1,y2-lines+1,x2,y2,bkcolor);
}
void hgVScrUpFast(int x1,int y1,int x2,int y2,int lines,int bkcolor)
{
        int xs=x1/8+1;
	int xe=x2/8-1;
        ...
        hgVBoxFill(xs*8,y2-lines+1,xe*8+7,y2,bkcolor);
}
